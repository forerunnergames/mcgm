...
Phone (web UI) ──HTTP──► Node.js bot
                          ...
                          ├─ Bisect/Pterodactyl API (server commands)
...
- **Server API** (`bisect.js`): Wraps the Bisect/Pterodactyl `/command` endpoint. Auto-tiles oversized fills, fuzzy-matches block names, validates against the block registry.
...
- A Bisect/Pterodactyl API key for the server
...
BISECT_API_KEY=ptlc_...
BISECT_API_BASE=https://games.bisecthosting.com/api/client
BISECT_SERVER_ID=your-server-id
...
Open `http://<your-mac-lan-ip>:3000` on your phone (same WiFi network).
